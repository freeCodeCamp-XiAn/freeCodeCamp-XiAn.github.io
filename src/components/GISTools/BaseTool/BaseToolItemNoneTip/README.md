...
> 该组件是基础组件的组成子项，相比于BaseToolItem组件该组件使用更方便但是不灵活，如果想灵活使用可以尝试BaseToolItem作为基础组件的组成子项。

## 属性

|名称|类型|必须|默认值|描述|
|--|--|--|--|--|
|title|string|是|无|对外显示的子项名称|
|key|number|是|无|组件必须的不唯一标识|
|imgPath|string|是|无|对外展示的子项图标地址|
|onClick|function|是|无|该子项的点击事件|

## 说明

该组件只提供了UI效果，引用该组件的组件需要做相应方法的实现
...
import distance from './img/distance.png' // 距离测量图标
