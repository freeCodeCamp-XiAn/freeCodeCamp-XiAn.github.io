	...
	contents: [
		{
			...
			project: ' React Hooks的使用',
			name: '熊彬--金数据前端工程师',
			description: '随着React 16.8 的正式发布，React Hooks也从提议照进了现实。我们从以前会把数据在每个组件都复写一遍，到开始使用HOC来减少代码数量。这一次，React为我们带来了一个新的解决方案，一起来了解一下React Hooks。涉及到的内容：* React 高阶组件(Higher- Order Components) * React Render props * React Hooks。',
			isImgLeft: false
		}, {
			project: '使用Typescript给JavaScript做静态类型检查',
			name: '欧阳英杰--ThoughtWorks咨询师',
			description: 'TypeScript，作为JavaScript的超集，给JavaScript带来了强类型这个非常强大的特性，给前端的开发以及重构带来了很大的便利性。但是，即使TypeScript现在已经可以直接使用用JavaScript编写的模块了，有很多遗留项目想要立马迁移到TypeScript也并非易事。但是好消息是，TypeScript在2.3版本引入了Js Type Checking，从此，不需要任何对于已有代码的侵入，你也可以很好的享受TypeScript带来的一些特性了。',
			isImgLeft: true
		}, {
			project: 'Tracup的前端性能监控实践',
			name: '吴阳--蒲公英研发部经理',
			...
			project: '从0搭建一个前端框架',
			name: '谷中仁--ThoughtWorks咨询师',
			description: '作为前端新趋势的 React 和 TypeScript 已经火的不要不要的了，作为前端er,  我们大多时候用脚手架做项目或者写测试，有木有想过从 0 开始搭建一个呢？本次分享，我将会分享从 0 搭建一个前端框架及其过程中遇到的坑,带你成为"前端架构师"。',
			isImgLeft: true
		}
	],
	corporateSponsor: [
