const Config = {
	...
	contentObj: {
		...
		name: '谷中仁',
		description:
			'从上面的代码可以看到将HTML直接嵌入了JS代码里面，这个就是React提出的一种JSX语法，这应该是最开始接触React最不能接受的设定之一，因为前端被"变现和逻辑层分离"这种思想"洗脑"太久了。但实际上组件的HTML是组成一个组件不可分割的一份，能够将HTML封装起来才是组件的完全体，React发明了JSX让JS支持嵌入...'
	},
	// 底图
	baseMapLayers: {
		title: '底图切换',
		baseLayers: [
			...
			// 天地图
			{
				...
				image: './img/image.png',
				url:
					'http://t{s}.tianditu.com/DataServer?T=img_c&x={x}&y={y}&l={z}'
			},
			{
				id: 'map',
				type: 'esri',
			...
			},
			{
				...
				image: './img/terrain.png',
				url:
					'http://t{s}.tianditu.com/DataServer?T=ter_c&x={x}&y={y}&l={z}'
			}
		]
	},
